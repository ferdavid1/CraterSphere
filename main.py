...
def main():
	...
	data = data[['LON_E', 'LAT', 'DiamKM']][:1000]
	lon, lat, diam = data['LON_E'], data['LAT'], data['DiamKM']
	# G = nx.MultiGraph()
	zipped = list(zip(lat, lon))
	# G.add_nodes_from(zipped[:1000])
	plt.scatter(lat, lon)
	inds = []
	for ind, (x,y) in enumerate(zipped):
		for x2,y2 in zipped:
			if neighboohood_dist(x,y,x2,y2):
				# G.add_edge((x,y), (x2,y2))
				plt.plot([x, x2], [y, y2], color='k', linestyle='-')
				# inds.append(ind)
			else:
				pass
	# diam = [diam[ind] for ind in inds]
	# print(len(list(G.edges())))
	# nx.draw(G, node_size=diam, with_labels=True)
	plt.show()
